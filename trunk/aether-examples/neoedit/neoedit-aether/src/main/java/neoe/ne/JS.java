...
public class JS {
	...
	public static List<StringBuffer> run(List<StringBuffer> lines, String userScript) throws Exception {
		...
		for (int i = 0; i < total; i++) {
			Object o = jsInvoke.invokeFunction("run", new Object[] { lines.get(i).toString(), i, total });
			addResult(o, res);
		}
		if (res.size() == 0) {
	...
	public static List<StringBuffer> runOnDir(String userScript) throws Exception {
		List<StringBuffer> res = new ArrayList<StringBuffer>();
		ScriptEngineManager manager = new ScriptEngineManager();
		ScriptEngine engine = manager.getEngineByName("js");
		engine.eval(userScript);
